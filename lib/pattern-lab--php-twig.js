...
module.exports = (gulp, config, tasks) => {
  ...
  function injectPl(done) {
    ...
    // The `bower_components.min.js` bundle is made in `js.js`.
    // It contains just dependencies (not devDeps) for Drupal etc.
    // We don't want to inject it in PL as all deps are added individually.
    sources = sources.concat(path.normalize(`!${config.js.dest}/bower_components.min.js`));
