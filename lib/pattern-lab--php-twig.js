...
module.exports = (gulp, config, tasks) => {
  ...
  function addTwigNamespaceConfigToPl(done) {
    ...
    const plConfigFile = yaml.safeLoad(
      fs.readFileSync(config.patternLab.configFile, 'utf8')
    );
    Object.assign(plConfigFile.plugins.twigNamespaces.namespaces, twigNamespaceConfig);
    const newConfigFile = yaml.safeDump(plConfigFile);
