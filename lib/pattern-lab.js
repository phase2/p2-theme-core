...
module.exports = function (gulp, config, tasks) {
  ...
  gulp.task('setup:pl', 'Setup Pattern Lab directories using Symlinks - only needed after "npm install"', function setupPl(cb) {
    ...
    if (config.patternLab.src.patternLabCss) {
      var cssDir = plnDir + 'public/styleguide/css';
      fs.removeSync(cssDir);
      fs.ensureSymlinkSync(config.patternLab.src.patternLabCss, cssDir);
    }
  ...
  gulp.task('clean:pl', 'Delete compiled Pattern Lab', function(done) {
    del([config.patternLab.dest]).then(function () {
      done();
    });
  });

  gulp.task('pl:css', 'Compile Pattern Lab\' CSS in ' + config.patternLab.src.patternLabCss, function() {
    return gulp.src(config.patternLab.src.patternLabCss + '*.scss')
      .pipe(sourcemaps.init())
      .pipe(sass({
        outputStyle: 'expanded'
      }).on('error', sass.logError))
      .pipe(gulp.dest(config.patternLab.src.patternLabCss));
  });

  gulp.task('watch:pl:css', function() {
    gulp.watch(config.patternLab.src.patternLabCss + '*.scss', ['pl:css']);
  });

  gulp.task('clean:pl:css', 'Delete compiled Pattern Lab CSS', function(done) {
    del([config.patternLab.src.patternLabCss + '*.css']).then(function () {
      done();
    });
  });
